...
**Version 2.0-draft Â· 17 September 2026 Â· HackSpain, 18â20 September 2026**
...
A case passes or it fails. There is no partial credit within a case â not for
...
submission at all, is always wrong â otherwise an agent that crashed would
...
that is currently open, dialled 10 at a time. You choose nothing about it â
...
transcript and no audio. Those open at the **reveal â Monday 21 September,
00:00 Europe/Madrid** â after the event has ended. The expected values are never
...
the fraction of its four cases that passed â 0, .25, .5, .75 or 1 â times that
weight. **Your score is the sum of those. There is no percentage and no
denominator.**

\`\`\`
points = sum over problems of (its pass fraction Ã its weight)
\`\`\`
...
problem it had not been built for â it would look like it was getting worse
...
luck â four cases per problem is a sample â so the board shows how many runs
...
Problem 2 scores nothing at all â it carries no weight and Run All never
...
Every call is capped at **three minutes** â an agent that cannot book in three
...
00:00 Europe/Madrid â after the stage final, so nothing can leak into it.
...
**Scored calls stay closed until the reveal â Monday 21 September, 00:00
...
Organisers are not on this clock â they can read any team's private-case detail
